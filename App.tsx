...
import { NarrativePanel, GeneratedNarrative } from './components/NarrativePanel';
import { AIDirectorPanel, DirectorConfig } from './components/AIDirectorPanel';
...
import { switchAIProvider, getActiveProviderType, ProviderType } from './services/client';
import { 
...
type ViewMode = 'FORGE' | 'ASSET_LAB' | 'NARRATIVE' | 'AI_DIRECTOR' | 'ANALYTICS';
type AppState = 'BOOT' | 'AUTH' | 'APP'; 
...
const PRESETS = {
  THREE_JS_FPS: {
    label: "3D Shooter",
    icon: Icons.Gamepad,
    description: "Three.js + Rapier + ECS",
    config: {
      gameEngine: GameEngine.ThreeJS,
      physicsEngine: PhysicsEngine.Rapier,
      architectureStyle: ArchitectureStyle.ECS,
      genre: Genre.FPS,
      cameraPerspective: CameraPerspective.FirstPerson,
      visualStyle: VisualStyle.Realistic,
      capabilities: { gpuTier: 'high', input: 'mouse', telemetry: true }
    }
  },
  PHYSICS_SANDBOX: {
    label: "Physics Lab",
    icon: Icons.Box,
    description: "WebGPU + Rapier Sim",
    config: {
      gameEngine: GameEngine.ThreeJS, // Stable fallback for now
      physicsEngine: PhysicsEngine.Rapier,
      architectureStyle: ArchitectureStyle.ECS,
      genre: Genre.Simulation,
      cameraPerspective: CameraPerspective.Orbital,
      visualStyle: VisualStyle.Minimalist,
      capabilities: { gpuTier: 'mid', input: 'mouse', telemetry: true }
    }
  },
  RETRO_2D: {
    label: "2D Arcade",
    icon: Icons.Ghost,
    description: "Kaboom.js + OOP",
    config: {
      gameEngine: GameEngine.KaboomJS,
      physicsEngine: PhysicsEngine.None, // Kaboom handles it
      architectureStyle: ArchitectureStyle.OOP,
      genre: Genre.Platformer,
      cameraPerspective: CameraPerspective.SideScroller,
      visualStyle: VisualStyle.Retro,
      capabilities: { gpuTier: 'low', input: 'mouse', telemetry: true }
    }
  }
};
...
const App: React.FC = () => {
  ...
  const handleLogin = (user: string) => {
    setOperator(user);
    setAppState('APP');
    addLog(`OPERATOR ${user.toUpperCase()} AUTHENTICATED`);
  };
  ...
  const [activeProvider, setActiveProvider] = useState<ProviderType>(getActiveProviderType());
  ...
  const handleProviderSwitch = (type: ProviderType) => {
      try {
          switchAIProvider(type);
          setActiveProvider(type);
          addLog(`SYSTEM: Switched Logic Engine to ${type}`);
      } catch (e: any) {
          setError({ title: "Config Error", message: e.message });
      }
  };
  ...
  const handleAssetAssigned = (asset: GeneratedAsset) => {
      setProjectAssets(prev => {
          ...
          if (viewMode === 'FORGE') addLog(`ASSET INJECTED: ${asset.role} (${asset.model})`);
          return [...prev, asset];
      });
  };
  ...
  const handleApplyPreset = (presetKey: keyof typeof PRESETS) => {
      ...
      if (btn) {
          btn.classList.add('ring-2', 'ring-indigo-500', 'bg-indigo-900/20');
          setTimeout(() => btn.classList.remove('ring-2', 'ring-indigo-500', 'bg-indigo-900/20'), 500);
      }
  };
  ...
  const handleDownloadAssets = async () => {
      ...
      const zip = new JSZip();
      const folder = zip.folder("project_assets");
      projectAssets.forEach((asset, i) => {
         if (asset.imageUrl) {
             const data = asset.imageUrl.split(',')[1];
             const name = `${asset.role}_${asset.id.slice(0,4)}_${i}.png`;
  ...
  const handleGeneratePrototype = async () => {
    ...
    try {
        ...
        const timeoutPromise = new Promise((resolve, reject) => {
            setTimeout(() => reject(new Error("Prototype generation timed out")), 180000); // Increased timeout
        });
  ...
  return (
    <div className="h-screen bg-black text-zinc-100 selection:bg-indigo-500/30 font-sans flex flex-col overflow-hidden">
      ...
      {showGlobalSettings && (
          <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowGlobalSettings(false)}>
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
                  <div className="flex justify-between items-center mb-6">
                      <h3 className="text-lg font-bold text-white flex items-center gap-2">
                          <Icons.Tools className="w-5 h-5 text-indigo-400" />
                          System Config
                      </h3>
                      <button onClick={() => setShowGlobalSettings(false)} className="text-zinc-500 hover:text-white">✕</button>
                  </div>
                  
                  <div className="space-y-6">
                      <div className="bg-zinc-950 p-4 rounded-lg border border-zinc-800">
                          <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider block mb-3">Logic Provider (Code Engine)</label>
                          <div className="grid grid-cols-2 gap-2">
                              <button 
                                onClick={() => handleProviderSwitch('OPENROUTER')}
                                className={`p-3 rounded-lg border text-sm font-bold flex flex-col items-center gap-2 ${activeProvider === 'OPENROUTER' ? 'bg-indigo-900/30 border-indigo-500 text-white' : 'bg-zinc-900 border-zinc-800 text-zinc-500 hover:bg-zinc-800'}`}
                              >
                                  <span>OpenRouter</span>
                                  {activeProvider === 'OPENROUTER' && <span className="text-[10px] bg-indigo-500 px-1.5 rounded text-white">ACTIVE</span>}
                              </button>
                              <button 
                                onClick={() => handleProviderSwitch('GEMINI')}
                                className={`p-3 rounded-lg border text-sm font-bold flex flex-col items-center gap-2 ${activeProvider === 'GEMINI' ? 'bg-indigo-900/30 border-indigo-500 text-white' : 'bg-zinc-900 border-zinc-800 text-zinc-500 hover:bg-zinc-800'}`}
                              >
                                  <span>Google Gemini</span>
                                  {activeProvider === 'GEMINI' && <span className="text-[10px] bg-indigo-500 px-1.5 rounded text-white">ACTIVE</span>}
                              </button>
                          </div>
                          <p className="text-[10px] text-zinc-600 mt-2">
                              Use OpenRouter for code generation (Gemini 3 Flash). Use Gemini direct for fallback.
                          </p>
                      </div>
                  </div>
              </div>
      ...
      <APIKeyManager
        ...
        onKeysUpdated={(keys) => {
          addLog(`API Keys updated: ${Object.keys(keys).join(', ')}`);
        }}
      />
      ...
      <div className="relative z-10 w-full h-full max-w-[1920px] mx-auto px-4 py-4 lg:px-6 flex flex-col">
              ...
              <div className="flex items-center gap-6">
                <h1 className="text-2xl font-black tracking-tight flex items-center gap-3">
                  ...
                  <div className="flex flex-col leading-none">
                    <span className="text-white">CANN.ON.AI</span>
                    <span className="text-indigo-500 text-sm font-bold">3DREAMFORGE</span>
                  </div>
                </h1>
                ...
                <div className="flex bg-zinc-900 rounded-lg p-1 border border-zinc-800">
                    ...
                    <button 
                        ...
                        className={`px-3 py-1.5 rounded-md text-xs font-bold uppercase tracking-wide transition-all ${
                            viewMode === 'NARRATIVE' ? 'bg-amber-600 text-white shadow-lg' : 'text-zinc-500 hover:text-white'
                        }`}
                    >
                        Narrative
                    </button>
                    <button 
                        onClick={() => setViewMode('AI_DIRECTOR')}
                        className={`px-3 py-1.5 rounded-md text-xs font-bold uppercase tracking-wide transition-all ${
                            viewMode === 'AI_DIRECTOR' ? 'bg-rose-600 text-white shadow-lg' : 'text-zinc-500 hover:text-white'
                        }`}
                    >
                        AI Director
                    </button>
                    <button 
                        ...
                        className={`px-3 py-1.5 rounded-md text-xs font-bold uppercase tracking-wide transition-all ${
                            viewMode === 'ANALYTICS' ? 'bg-emerald-600 text-white shadow-lg' : 'text-zinc-500 hover:text-white'
                        }`}
                    >
                        Analytics
                    </button>
              ...
              <div className="flex items-center gap-4">
                ...
                {operator && (
                  <div className="text-[10px] text-zinc-500 font-mono">
                    OP: <span className="text-white font-bold uppercase">{operator}</span>
                  </div>
                )}
              </div>
          </div>
        ...
        <div className="flex-1 min-h-0 relative">
            {viewMode === 'ASSET_LAB' ? (
                ...
                <div className="h-full w-full animate-in fade-in">
                    <NarrativePanel 
                        gameConcept={prefs.projectDescription || 'A mysterious adventure awaits...'}
                        genre={prefs.genre}
                    />
                </div>
            ) : viewMode === 'AI_DIRECTOR' ? (
                <div className="h-full w-full animate-in fade-in">
                    <AIDirectorPanel 
                        isActive={true}
                    />
                ...
                <div className="h-full grid grid-cols-1 lg:grid-cols-12 gap-6 pb-2">
                    ...
                    <div className="lg:col-span-3 flex flex-col gap-4 overflow-y-auto pr-2 custom-scrollbar">
                        ...
                        {/* PROJECT INVENTORY */}
                        {projectAssets.length > 0 && !specFrozen && (
                            <div className="mb-4 bg-zinc-950/50 border border-zinc-800 rounded-lg p-3">
                                <div className="flex items-center justify-between mb-2">
                                    <h3 className="text-[10px] font-bold text-green-400 uppercase tracking-widest flex items-center gap-2">
                                        <Icons.Box className="w-3 h-3" /> Project Inventory
                                    </h3>
                                    <div className="flex items-center gap-2">
                                        <span className="text-[9px] text-zinc-500">{projectAssets.length} Assets</span>
                                        <button 
                                ...
                                <div className="grid grid-cols-4 gap-1">
                                    {projectAssets.map((a, i) => (
                                        <div key={i} className="aspect-square bg-zinc-900 rounded overflow-hidden relative group cursor-pointer border border-zinc-800" title={a.role + ": " + a.prompt}>
                                            <img src={a.spriteSheetUrl || a.imageUrl} className="w-full h-full object-cover opacity-70 group-hover:opacity-100" />
                                            <div className="absolute bottom-0 inset-x-0 bg-black/70 text-[8px] text-white truncate px-1 py-0.5">
                                                {a.role}
                                            </div>
                                        </div>
                                    ))}
                        ...
                        {phase === 'idle' && !specFrozen && (
                            <div className="mb-4 space-y-2">
                                <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Quick Start Templates</h3>
                                <div className="grid grid-cols-3 gap-2">
                                    {Object.entries(PRESETS).map(([key, preset]) => (
                                        <button
                                            ...
                                            className="flex flex-col items-center justify-center p-3 bg-zinc-950 border border-zinc-800 rounded-lg hover:bg-zinc-800 hover:border-indigo-500/50 transition-all group shadow-sm hover:shadow-md"
                                            title={preset.description}
                                        >
                                            <preset.icon className="w-5 h-5 text-zinc-500 group-hover:text-indigo-400 mb-2 transition-colors" />
                                            <span className="text-[10px] font-bold text-zinc-400 group-hover:text-white text-center leading-tight">{preset.label}</span>
                                        </button>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className={`space-y-4 ${specFrozen ? 'opacity-30 pointer-events-none grayscale' : (phase !== 'idle' ? 'opacity-50 pointer-events-none' : '')}`}>
                            <div className="space-y-3 pb-3 border-b border-zinc-800/50">
                                <h3 className="text-[10px] font-bold text-zinc-200 flex items-center gap-2"><Icons.Cpu className="w-3 h-3" /> Architecture</h3>
                            ...
                            <div className="space-y-3 pb-3 border-b border-zinc-800/50">
                                <h3 className="text-[10px] font-bold text-zinc-200 flex items-center gap-2"><Icons.Monitor className="w-3 h-3" /> Visuals & Atmosphere</h3>
                                <div className="grid grid-cols-1 gap-2">
                            ...
                            <div className="pt-3 border-t border-zinc-800/50">
                                <div className="flex items-center justify-between mb-2">
                                    <h3 className="text-[10px] font-bold text-zinc-200 flex items-center gap-2">
                                        <span>🎮</span> Modifiers
                                    </h3>
                                    <button
                                        onClick={() => setShowModifierPicker(true)}
                                        className="text-[10px] text-indigo-400 hover:text-indigo-300 underline"
                                    >
                                        {selectedModifiers.length > 0 ? `Edit (${selectedModifiers.length})` : 'Add Modifiers'}
                                    </button>
                                </div>
                                {selectedModifiers.length > 0 && (
                                    <div className="flex flex-wrap gap-1">
                                        {selectedModifiers.slice(0, 5).map((modId) => (
                                            <span key={modId} className="px-2 py-0.5 bg-indigo-500/20 text-indigo-300 text-[10px] rounded">
                                                {modId}
                                            </span>
                                        ))}
