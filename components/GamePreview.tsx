...
export const GamePreview: React.FC<Props> = ({ html, title }) => {
  ...
  return (
    <div ref={containerRef} className={`flex flex-col bg-zinc-900 relative ${isFullscreen ? 'fixed inset-0 z-[9999]' : 'w-full h-full'}`}>
      {/* Header - Made slimmer */}
      <div className="flex items-center justify-between px-3 py-1.5 bg-zinc-950 border-b border-zinc-800 z-10 shrink-0">
        ...
        <div className="flex gap-1">
            ...
            <button onClick={toggleFullscreen} className="p-1 hover:bg-zinc-800 rounded text-zinc-400 hover:text-white" title={isFullscreen ? 'Exit Fullscreen' : 'Fullscreen'}>
      ...
      <div className="relative flex-1 w-full bg-black group overflow-hidden">
        ...
        <iframe
          ...
          // We must be careful with sandbox in srcDoc. 'allow-same-origin' is needed for some engine features but can be risky.
          // 'allow-scripts' is mandatory. 'allow-pointer-lock' is needed for FPS games.
