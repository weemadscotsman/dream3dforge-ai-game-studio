...
const App: React.FC = () => {
  ...
  const [operator, setOperator] = useState<string>('Guest');
  ...
  const handleApplyPreset = (presetKey: keyof typeof PRESETS) => {
      ...
      setPrefs(prev => ({
          ...prev,
          ...(preset.config as unknown as Partial<UserPreferences>), // Fix type mismatch
  ...
  const handleExport = async () => {
      ...
      const readme = `# ${game.title}\n\n${game.summary}\n\nGenerated by CANN.ON.AI 3DREAMFORGE`;
  ...
  return (
    <div className="h-screen bg-black text-zinc-100 selection:bg-indigo-500/30 font-sans flex flex-col overflow-hidden animate-in fade-in duration-500">
                  ...
                  <div className="space-y-6">
                      <div className="bg-zinc-950 p-4 rounded-lg border border-zinc-800">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                              <button 
                                onClick={() => handleProviderSwitch('KIMI')}
                                className={`p-3 rounded-lg border text-sm font-bold flex flex-col items-center gap-2 ${activeProvider === 'KIMI' ? 'bg-indigo-900/30 border-indigo-500 text-white' : 'bg-zinc-900 border-zinc-800 text-zinc-500 hover:bg-zinc-800'}`}
                              >
                                  <span>Moonshot Kimi</span>
                                  {activeProvider === 'KIMI' && <span className="text-[10px] bg-indigo-500 px-1.5 rounded text-white">ACTIVE</span>}
                              </button>
                              <button 
                          ...
                          <p className="text-[10px] text-zinc-600 mt-2">
                              Use Kimi for massive context windows (128k). Use Gemini for multi-modal tasks.
                          </p>
                      </div>
                  </div>
              </div>
