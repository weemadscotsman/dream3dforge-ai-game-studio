        ...
        <div className="flex flex-col gap-3">
             ...
             <div className="grid grid-cols-2 gap-4">
                 <div>
                    ...
                    <select 
                        value={props.model} 
                        onChange={(e) => props.setModel(e.target.value as ModelType)} 
                        className="w-full bg-zinc-950 border border-zinc-800 rounded p-2 text-xs text-white outline-none focus:border-indigo-500 transition-colors"
                    >
          ...
          {showAdvanced && (
              <div className="mt-3 space-y-3 animate-in fade-in slide-in-from-top-2">
                  <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                          <label className="text-[9px] text-zinc-500 uppercase font-bold block mb-1">Top P</label>
                          <input 
                            type="number" step="0.05" min="0" max="1" 
                            value={props.advancedConfig.topP} 
                            onChange={(e) => props.setAdvancedConfig({...props.advancedConfig, topP: parseFloat(e.target.value)})}
                            className="w-full bg-zinc-950 border border-zinc-800 rounded p-1.5 text-xs text-white"
                          />
                      </div>
                      <div>
                          <label className="text-[9px] text-zinc-500 uppercase font-bold block mb-1">Top K</label>
                          <input 
                            type="number" step="1" min="1" max="128" 
                            value={props.advancedConfig.topK} 
                            onChange={(e) => props.setAdvancedConfig({...props.advancedConfig, topK: parseFloat(e.target.value)})}
                            className="w-full bg-zinc-950 border border-zinc-800 rounded p-1.5 text-xs text-white"
                          />
                      </div>
                  </div>
