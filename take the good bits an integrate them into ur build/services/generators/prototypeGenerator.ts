...
export const generatePrototype = async (
  ...
  // --- ASSET PROXY SYSTEM ---
  // We extract heavy Base64 data here and replace it with lightweight JS variable references for the AI Prompt.
  const assetPayload: Record<string, string> = {};
  const proxyAssets: any[] = []; // Lightweight for Prompt

  if (assignedCount > 0) {
      log(`Linking ${assignedCount} Custom Asset(s) via Proxy Injection...`);
      
      assets.filter(a => a.role).forEach((asset, index) => {
          const safeId = asset.id.replace(/-/g, '_') || `asset_${index}`;
          const mainKey = `ASSET_${safeId}_MAIN`;
          
          // Determine best source (Sheet > Image)
          const dataUrl = (asset.isSpriteSheet && asset.spriteSheetUrl) ? asset.spriteSheetUrl : (asset.imageUrl || "");
          
          if (dataUrl) {
              // 1. Store Heavy Data (Albedo)
              assetPayload[mainKey] = dataUrl;
              
              // 2. Create Lightweight Proxy
              const proxy: any = {
                  ...asset,
                  // The AI sees a JS variable reference instead of the massive Base64 string
                  imageUrl: `window.FORGE_ASSETS['${mainKey}']`, 
                  description: `${asset.prompt} (${asset.isSpriteSheet ? 'Sprite Sheet' : 'Static'})`,
              };

              // Handle Maps (Normal)
              if (asset.normalMapUrl) {
                  const normKey = `ASSET_${safeId}_NORM`;
                  assetPayload[normKey] = asset.normalMapUrl;
                  proxy.normalMapUrl = `window.FORGE_ASSETS['${normKey}']`;
              }

              // Handle Maps (Roughness)
              if (asset.roughnessMapUrl) {
                  const roughKey = `ASSET_${safeId}_ROUGH`;
                  assetPayload[roughKey] = asset.roughnessMapUrl;
                  proxy.roughnessMapUrl = `window.FORGE_ASSETS['${roughKey}']`;
              }

              // Handle Maps (ORM for UE5 style logic in standard materials)
              if (asset.ormMapUrl) {
                  const ormKey = `ASSET_${safeId}_ORM`;
                  assetPayload[ormKey] = asset.ormMapUrl;
                  proxy.ormMapUrl = `window.FORGE_ASSETS['${ormKey}']`;
              }

              proxyAssets.push(proxy);
          }
      });
  }
  ...
  try {
    const prototypeData = await withRetry(async () => {
        ...
        // We pass 'proxyAssets' to the prompt registry, not the full heavy assets
        const response = await ai.generateContent({
          model: "gemini-3-pro-preview",
          contents: PromptRegistry.BuildPrototype(blueprint, prefs, audioContextStr, gpuRules, proxyAssets),
          config: {
    ...
    // --- RE-INJECT HEAVY ASSETS INTO HTML ---
    log("Injecting Binary Asset Payload...");
    
    // Create a script block that defines the global asset store
    // We place this at the very top of the head so it is available immediately
    const assetInjectionScript = `
    <script>
      // DREAM3D FORGE ASSET PAYLOAD
      // These assets are injected via proxy references to save token context during generation.
      window.FORGE_ASSETS = ${JSON.stringify(assetPayload)};
      console.log("Forge Assets Loaded:", Object.keys(window.FORGE_ASSETS).length);
    </script>
    `;
    
    // Inject just after <head> or at the start if no head (unlikely)
    let finalHtml = prototypeData.html;
    if (finalHtml.includes("<head>")) {
        finalHtml = finalHtml.replace("<head>", `<head>${assetInjectionScript}`);
    } else {
        finalHtml = `${assetInjectionScript}\n${finalHtml}`;
    }

    // Generate Manifest
    log("Signing build manifest...");
    const specHash = generateShortHash(JSON.stringify(blueprint));
    const buildHash = generateShortHash(finalHtml);
